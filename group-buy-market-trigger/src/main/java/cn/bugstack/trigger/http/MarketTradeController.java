...
public class MarketTradeController implements IMarketTradeService {
    ...
    public Response<LockMarketPayOrderResponseDTO> lockMarketPayOrder(@RequestBody LockMarketPayOrderRequestDTO lockMarketPayOrderRequestDTO) {
        try {
            ...
            if (null != marketPayOrderEntity) {
                ...
                log.info("交易记录存在：{} marketPayOrderEntity:{}",userId,marketPayOrderEntity);
                return Response.<LockMarketPayOrderResponseDTO>builder()
            ...
            if(null!=teamId)
            {
                ...
                {
                    log.info("交易目标已经达成{},{}",userId,teamId);
                    return Response.<LockMarketPayOrderResponseDTO>builder()
            ...
            TrialBalanceEntity trialBalanceEntity = indexGroupByMarketService.indexMarketTrial(MarketProductEntity
                    .builder()
                    .userId(userId)
                    .goodsId(goodsId)
                    .source(source)
    ...
    public Response<SettlementMarketPayOrderResponseDTO> settlementMarketPayOrder(@RequestBody SettlementMarketPayOrderRequestDTO requestDTO) {
        try {
            ...
            //1.调用结算服务 将requestDTO转换为领域层的Entity冰
            TradePaySettlementEntity tradePaySettlementEntity = tradeSettlementOrderService.settlementMarketPayOrder(TradePaySuccessEntity.builder().
                    ...
                    .source(requestDTO.getSource()).outTradeNo(requestDTO.getOutTradeNo()).outTradeTime(requestDTO.getOutTradeTime()).build()
            );
            ...
            Response<SettlementMarketPayOrderResponseDTO> response=Response.<SettlementMarketPayOrderResponseDTO>builder()
                    .code(ResponseCode.SUCCESS.getCode())
                    .info(ResponseCode.SUCCESS.getInfo())
                    .data(settlementMarketPayOrderResponseDTO).build();

            log.info("营销交易组队结算完成:{} outTradeNo :{},response:{}",requestDTO.getUserId(),requestDTO.getOutTradeNo(),response);
            return response;
        } catch (AppException e) {
